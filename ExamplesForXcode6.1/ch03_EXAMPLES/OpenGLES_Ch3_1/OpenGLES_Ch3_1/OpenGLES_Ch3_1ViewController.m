...
- (void)viewDidLoad
{
   ...
   CGImageRef imageRef = 
    ...
    /**
     `textureWithCGImage:`方法会接受一个 CGImageRef并创建一个新的包含CGImageRef的像素数据的 OpenGL ES 纹理缓存，
     option参数接受一个存储了用于指定GLKTextureLoader怎么解析加载的图像数据的键值对的NSDictionary,可选项之一是指示GLKTextureLoader为加载的图像生成 MIP 贴图
